...
### Notes / gotchas
- **Network: WIRED-ONLY installer (hardened after the first build, 2026-07-08).** The G11's 2.5GbE
  Realtek **RTL8125** relinks slowly; the first stick let stock d-i decide the wired port was "dead"
  after a couple seconds and **bail to Wi-Fi**, which turned an unattended install into a painful
  Wi-Fi-config dead-end. The installer now **cannot use Wi-Fi at all**:
    - `build-seed-iso.sh` boots with **`modprobe.blacklist=mt7921e`** → the MediaTek Wi-Fi driver never
      loads in the installer, so `netcfg` has only wired ports to pick. (Installer-only — the installed
      system loads `mt7921e` normally; verified on `.210` where `wlp2s0` still enumerates.)
    - `early_command` ups every wired NIC **and actively waits up to 45 s for a wired carrier** before
      `netcfg` probes; `choose_interface=auto` + `link_wait_timeout 60` then take the wired port and DHCP,
      fully unattended.
  **If a box genuinely has no wired link:** at the boot menu edit the entry (grub: `e`; isolinux: `Tab`)
  and delete the `modprobe.blacklist=mt7921e` token to re-enable Wi-Fi (it then prompts for SSID +
  passphrase at `priority=high`; note d-i can't join a **WPA3-only** SSID — use WPA2/mixed). Or preseed a
  **static wired IP** (`netcfg/disable_autoconfig true` + `get_ipaddress/netmask/gateway/nameservers`).
