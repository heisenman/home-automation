...
| `gatt_exec` / `gatt_history` | Server-driven GATT actuation / history pull | ADR-0010 | native-radio **or** VHCI | NimBLE central | **fork (byte-identical ×3)** — `gatt_exec` NOT a plain promote: it `#include`s `ha_mqtt.h`+`ble_scan.h`+`secrets.h` (board-local), so it needs an `ha_ota`-style cfg seam before extraction (`edge-core-dedup`). `gatt_history` → shared [`ha_gatt`](../firmware/components/ha_gatt/): c6, c3, panel wired; **s3-eth pending** (live node — flash-gated) |
