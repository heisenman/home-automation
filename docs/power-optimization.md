...
| 2026-06-27 | **Day-2 interim read** (749 samples, 2026-06-25 00:55 → 06-27 17:03) | RAPL pkg + C-state deltas | n/a (measurement) | **pkg p50 4.29 W · p95 5.05 W · mean 4.37 W · min 3.77; idle C2 98.6%; temp 43–53 °C (p50 47).** ~89% of samples in 4.0–4.5 W bins. ~38 kWh/yr pkg-only | — | — | this box |
| 2026-06-27 | The watt spread is almost entirely the **dev AI agent**, not the HA workload | every high-W sample's top proc = `claude` (~27% CPU); only 40/749 (5%) samples "active", mostly agent-driven; real HA steady-state ≈ 4.0–4.3 W mode (mean inflated by profiling activity) | none needed — informs interpretation; clean prod baseline wants an agent-quiet window | true prod idle ~4.0–4.3 W; "services are cheap" confirmed | — | — | this box (agents are dev-only) |
