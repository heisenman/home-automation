...
| `sgp30` | Sensirion SGP30 eCO₂+TVOC sensor I2C driver (0x58; measure/self-test/baseline). Computes eCO₂/TVOC on-chip → NO `sensirion_gas_index` companion needed (cf. `sgp40`@0x59) | ADR-0020 | any node with an I2C bus (bus/pins injected) | driver (i2c_master) | **shared → [firmware/components/](../firmware/components/sgp30/)** — for `Cbed_C6_gsens`; **not yet consumed** by a build (awaits ha_gas SGP30 wiring) |
