...
#include "freertos/FreeRTOS.h"   // ADAPTED: the legacy driver/i2c.h used to pull FreeRTOS in transitively;
#include "freertos/task.h"       // the new i2c_master.h does not, and es8311_init() uses vTaskDelay.

// ADAPTED (ha_audio): the panel drives I2C_1 with the NEW i2c_master driver (bsp_i2c1()), so this device
// holds an i2c_master_dev_handle_t instead of the vendor original's (i2c_port_t + addr) for the legacy
// driver. Only the struct, es8311_create(), and the two read/write primitives below differ from the
// upstream espressif__es8311 v1.0.0~1 driver — all register logic + the coeff table are verbatim.
typedef struct {
...
static inline esp_err_t es8311_write_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t data)
{
    ...
    return i2c_master_transmit(es->dev, write_buf, sizeof(write_buf), 1000);
}

static inline esp_err_t es8311_read_reg(es8311_handle_t dev, uint8_t reg_addr, uint8_t *reg_value)
{
    es8311_dev_t *es = (es8311_dev_t *) dev;
    return i2c_master_transmit_receive(es->dev, &reg_addr, 1, reg_value, 1, 1000);
}
...
es8311_handle_t es8311_create(i2c_master_bus_handle_t bus, const uint16_t dev_addr)
{
    ...
    if (i2c_master_bus_add_device(bus, &dc, &sensor->dev) != ESP_OK) { free(sensor); return NULL; }
