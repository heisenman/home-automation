// GATT-central client for SwitchBot on-device history. Promoted verbatim-in-behaviour from
// edge/esp32c6/main/gatt_history.c (roadmap #5): the reverse-engineered handshake/paging (see
// tools/switchbot_history.py + docs) is unchanged; the only edits vs. the edge original are the two
// platform seams — the relay sink and the diagnostic log are now cfg callbacks (ha_gatt_init) instead of
// direct ha_mqtt_* calls, and the shared scan is ha_ble_scan (which the edge original already used).
// Single radio: the passive scan is paused for the pull and resumed on disconnect.
#include "ha_gatt.h"
...
static const char *TAG = "ha_gatt";

// ── Platform seams (ha_gatt_init) ────────────────────────────────────────────────
static ha_gatt_cfg_t s_cfg;
void ha_gatt_init(const ha_gatt_cfg_t *cfg) { if (cfg) s_cfg = *cfg; }

// Diagnostic log line -> cfg.log (was ha_mqtt_log on the edge). Always mirrored to ESP_LOG.
static void gatt_log(const char *fmt, ...)
{
    char b[160];
...
// Relay one JSON line -> cfg.publish (was ha_mqtt_publish_history on the edge).
static void gatt_publish(const char *mac_str, const char *json)
{
    if (s_cfg.publish) s_cfg.publish(mac_str, json, s_cfg.user);
}
...
static struct {
    ...
    uint32_t pull_now;
    int window_records;                // 0 = full range; >0 = pull only the most-recent N records
    // relay batch (guarded by s_batch_mutex)
    char batch[1024];
    int batch_count, total_count, seq;
} g;
static SemaphoreHandle_t s_batch_mutex;

bool ha_gatt_busy(void) { return s_busy; }
...
static void relay_record(const uint8_t *data, int len) {
    ...
    if (send) gatt_publish(g.mac_str, payload);   // outside the lock
}
...
static void flush_remaining(void) {
    ...
    xSemaphoreGive(s_batch_mutex);
    if (send) gatt_publish(g.mac_str, payload);
}
...
static int on_chr(uint16_t conn, const struct ble_gatt_error *err, const struct ble_gatt_chr *chr, void *arg) {
    if (chr) {
        ...
        gatt_log("  chr val=%u %s", chr->val_handle, u);
...
static int on_svc(uint16_t conn, const struct ble_gatt_error *err, const struct ble_gatt_svc *svc, void *arg) {
    if (svc) {
        ...
        gatt_log("svc %u-%u %s", svc->start_handle, svc->end_handle, u);
    ...
    if (err && err->status == BLE_HS_EDONE) {
        ...
        else { gatt_log("target service NOT found"); xEventGroupSetBits(s_evt, BIT_FAIL); }
        return 0;
    }
    gatt_log("svc disc err=%d", err ? err->status : -1);
    xEventGroupSetBits(s_evt, BIT_FAIL);
    return 0;
}
...
static int conn_event(struct ble_gap_event *event, void *arg) {
    ...
    case BLE_GAP_EVENT_CONNECT:
        gatt_log("connect status=%d", event->connect.status);
    ...
    case BLE_GAP_EVENT_DISCONNECT:
        gatt_log("disconnect reason=%d", event->disconnect.reason);
    ...
    case BLE_GAP_EVENT_CONN_UPDATE:
        gatt_log("conn update: status=%d", event->conn_update.status);
        return 0;
...
static void publish_meta(void) {
    ...
    if (g.prof == &PROF_OUTDOOR) {
        ...
        snprintf(p, sizeof(p),
            ...
            g.oldest_ptr, (unsigned)g.start_addr, (unsigned)g.pull_now);
    }
    gatt_publish(g.mac_str, p);
}
...
static const char *outdoor_pull(void) {
    ...
    for (int bank = 0; bank <= 3; bank++) {
        ...
        gatt_log("bank %d: meta=%d ptr=%u ts=%u", bank, (int)g.probe_got, (unsigned)g.probe_ptr,
                 (unsigned)g.probe_ts);
    ...
    gatt_log("outdoor bank=%d newest_addr=%u ts=%u", best_bank, (unsigned)best_ptr, (unsigned)best_ts);

    // ~2048 records (~34h) bounds pull time by default; a shared-radio caller can pass a smaller
    // window_records so the (slower, coexistence-throttled) pull completes before a link-layer timeout.
    uint32_t win_recs = (g.window_records > 0 && g.window_records < 2048) ? (uint32_t)g.window_records : 2048u;
    const uint32_t WINDOW = win_recs * REC_STRIDE;
...
static void finish(const char *why) {
    gatt_log("pull end: %s (relayed %d notifs)", why, g.total_count);
...
static void pull_task(void *arg) {
    ...
    if (!g.cmd_handle || !g.notify_handle) {
        gatt_log("discovery fail: cmd=%u notify=%u", g.cmd_handle, g.notify_handle);
        finish("discovery failed"); vTaskDelete(NULL); return;
    }
    gatt_log("discovered: cmd=%u notify=%u", g.cmd_handle, g.notify_handle);
    ...
    for (uint16_t addr = start; addr < g.newest_ptr; addr += REC_STRIDE) {
        cmd[rp]   = (addr >> 8) & 0xff;
        cmd[rp+1] = addr & 0xff;
        cmd[rp+2] = 0x06;
...
bool ha_gatt_history_pull(const char *mac_str, const char *profile, int window_records) {
    ...
    g.window_records = window_records;
    ...
    if (!ha_ble_lookup_addr(mac_str, &g.addr)) {
        gatt_log("pull %s: addr not cached by scanner — can't connect", mac_str);
        return false;
    }
    s_busy = true;
    ...
    gatt_log("pull %s: connecting (addr_type=%d, profile=%s)", mac_str, g.addr.type,
    ...
    if (rc != 0) {
        gatt_log("pull %s: ble_gap_connect rc=%d", mac_str, rc);
...
// ── Probe (Spike-0 diagnostic): connect + discover services only ─────────────────
// Reuses conn_event (sets BIT_CONNECTED / g.conn) but counts ALL services via probe_on_svc rather than
// hunting the SwitchBot UUID. Shares s_busy so it can't run concurrently with a pull.
static volatile int s_probe_nsvc;
static int probe_on_svc(uint16_t conn, const struct ble_gatt_error *err, const struct ble_gatt_svc *svc, void *arg) {
    (void)conn; (void)arg;
    if (svc) {
        char u[BLE_UUID_STR_LEN]; ble_uuid_to_str(&svc->uuid.u, u);
        gatt_log("  svc %u-%u %s", svc->start_handle, svc->end_handle, u);
    ...
    xEventGroupSetBits(s_evt, BIT_DISCOVERED);   // BLE_HS_EDONE (or any terminal status)
    return 0;
}
...
static void probe_task(void *arg) {
    ...
    EventBits_t bits = xEventGroupWaitBits(s_evt, BIT_CONNECTED | BIT_FAIL | BIT_DISCONNECT,
                                           pdTRUE, pdFALSE, pdMS_TO_TICKS(12000));
    if (!(bits & BIT_CONNECTED)) { gatt_log("probe %s: connect FAILED (central role unavailable?)", g.mac_str); goto done; }
    gatt_log("probe %s: CONNECTED — discovering services…", g.mac_str);
    vTaskDelay(pdMS_TO_TICKS(400));   // settle (avoids the ENOTCONN race)
    s_probe_nsvc = 0;
    ble_gattc_disc_all_svcs(g.conn, probe_on_svc, NULL);
    bits = xEventGroupWaitBits(s_evt, BIT_DISCOVERED | BIT_DISCONNECT, pdTRUE, pdFALSE, pdMS_TO_TICKS(8000));
    if (bits & BIT_DISCOVERED)
        gatt_log("probe %s: OK — %d services (GATT CENTRAL over HCI WORKS)", g.mac_str, s_probe_nsvc);
    else
        gatt_log("probe %s: discovery timed out / disconnected", g.mac_str);
    ble_gap_terminate(g.conn, BLE_ERR_REM_USER_CONN_TERM);
    xEventGroupWaitBits(s_evt, BIT_DISCONNECT, pdTRUE, pdFALSE, pdMS_TO_TICKS(3000));
done:
    ha_ble_scan_resume();
...
bool ha_gatt_probe(const char *mac_str) {
    if (s_busy) { gatt_log("probe busy"); return false; }
    if (!s_evt) { s_evt = xEventGroupCreate(); s_write_sem = xSemaphoreCreateBinary(); s_batch_mutex = xSemaphoreCreateMutex(); }
    memset(&g, 0, sizeof(g));
    snprintf(g.mac_str, sizeof(g.mac_str), "%s", mac_str);
    if (!ha_ble_lookup_addr(mac_str, &g.addr)) { gatt_log("probe %s: addr not cached (not heard advertising yet)", mac_str); return false; }
    s_busy = true;
    xEventGroupClearBits(s_evt, 0xFF);
    ha_ble_scan_pause();
    gatt_log("probe %s: connecting (addr_type=%d)…", mac_str, g.addr.type);
    int rc = ble_gap_connect(ha_ble_own_addr_type(), &g.addr, 10000, &FAST_CONN, conn_event, NULL);
    if (rc != 0) {
        gatt_log("probe %s: ble_gap_connect rc=%d", mac_str, rc);
        ha_ble_scan_resume(); s_busy = false; return false;
    }
