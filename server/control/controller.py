...
class Controller:
    def __init__(self, issuer, drivers: dict, registry: dict, db: str, mqtt_client=None):
        ...
        self.readings: dict[str, Reading] = {}  # sensor device_id -> latest Reading
    ...
    def on_message(self, client, userdata, msg):
        try:
            ...
            return
        did = p.get("device_id")
        hum = (p.get("metrics") or {}).get("humidity_pct")
        if did and hum is not None:
            with self._lock:
                self.readings[did] = Reading(float(hum), time.time())
    ...
    def _tick_device(self, conn, device_id, pol, now, tod, dry_run):
        ...
        if drv is None:
            return
        try:
            st = drv.status()                                  # live interlocks + state
