...
#define APP_BUILD_TAG "v109-mapflicker"
...
// ADR-0030 recovery is now owned entirely by the 2nd-stage bootloader (bootloader_components/main):
// a 3s GPIO3 high-hold boots the IMMUTABLE golden `test` partition, and it covers crash-at-boot (the app
// never runs). The former app-level gate (A2, esp_ota_set_boot_partition to ota_1) was retired — it can't
// target a non-ota `test` partition and the bootloader path is strictly more robust. See recovery.h +
// docs/design/panel-recovery-findings.md.
